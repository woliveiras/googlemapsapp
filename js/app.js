...
function addPoint() {
  ...
  if (counter <= 8) {
    $newWayPoint.appendTo($waypoint).addClass('waypoint');
    $newWayPoint.find('input').val("").focus().addClass('new-point')
      .attr("placeholder", "Ponto").attr("id", "waypoint_" + counter)
      .attr("name", "waypoint_" + counter)
      .after("<button class='btn-remove' id='btnDelPoint' name='btnDelPoint'>Remover</button>");
    ...
    if (counter <= 3) {
      $("#btnAddPoint")
        .after("<button class='btn-clear' id='btnClearPoints' name='btnClearPoints'>Remover pontos adicionados</button>");
    };
    return false;
  };
};
